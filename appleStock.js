...
function getMaxProfit(stockPrices) {
  ...
  // store current min and the max profit
  let currentMinBuy = stockPrices[0];
  let maxProfit = -Infinity;

  // iterate through stockPrices and update above variables as needed
  for (let i = 1; i < stockPrices.length; i++) {
    let sellNowProfit = stockPrices[i] - currentMinBuy;
    if (sellNowProfit > maxProfit) {
      maxProfit = sellNowProfit;
    }
    if (stockPrices[i] < currentMinBuy) {
      currentMinBuy = stockPrices[i];
    }
  }
