...
function isBalanced(treeRoot) {
  ...
  // a tree with no nodes is automatically superbalanced!!
  if (!treeRoot) {
    return true;
  }

  // keep track of depths of each leaf
  const depths = [];

  // stack structure to keep track of nodes in DFS traversal of tree
  const nodes = [];
  nodes.push([treeRoot, 0]); // each node in the nodes stack with consist of node at index 0 and depth of that node at index 1
  ...
  while (nodes.length) {
    ...
    // check to see if this particular node is a leaf (no children)
    if (!node.left && !node.right) {
      // check to see if this is a new depth to add to our depth array
      if (depths.indexOf(depth) < 0) {
        ...
        //    2: if 2 depths, they differ by more than 1
        if ((depths.length > 2) 
    ...
    // if there are children nodes, push them onto the node stack
    if (node.left) {
